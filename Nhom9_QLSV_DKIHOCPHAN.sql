...
--Trigger 1: Không cho phép sinh viên có vi phạm đăng ký môn học
...
go
INSERT INTO SINHVIEN VALUES ('20132202', N'Phạm Hồng Hiếu', N'Nam', '2002-06-01', '201321C' , '12000')
DELETE FROM SINHVIEN WHERE MaSV = '20132202'
INSERT INTO DANGKY VALUES ('20132202','TAPO330407_01')


--Trigger 2: Không cho một phòng tại 1 thời điểm có nhiều lớp học
...
go
INSERT INTO LOPHOC VALUES ('1114', 'MHDT009', '1951', '60', 'A111', 'Thu 7', 5, 6, '2022-06-01', '2022-12-01', '1', '2022')

--Trigger 4: Không cho phép trùng môn học đào tạo
...
go
INSERT INTO MONHOC_DAOTAO VALUES ('MHDT102', 'TAPO330407', 'CTDT4', '457')



--Trigger 5: Không cho phép sinh viên đăng kí một môn nhiều lớp
...
go
exec Re_DangKyLH 'LLCT120314_02', '20151426'
...
-----------------------------------BackUp-----------------------------
BACKUP DATABASE QUANLYSV
TO DISK = 'C:\Users\Admin\Desktop\kk\Full.bak'

BACKUP DATABASE QUANLYSV
TO DISK = 'C:\Users\Admin\Desktop\kk\Diff.bak'
WITH DIFFERENTIAL

BACKUP LOG QUANLYSV 
TO DISK = 'C:\Users\Admin\Desktop\kk\Log.bak'

--------------------------------------------------------------------Restore--------------------------------------------------
RESTORE DATABASE QUANLYSV 
FROM DISK = 'C:\Users\Admin\Desktop\kk\Full.bak'
WITH NORECOVERY

RESTORE DATABASE QUANLYSV 
FROM DISK = 'C:\Users\Admin\Desktop\kk\Diff.bak'
WITH NORECOVERY

RESTORE LOG QUANLYSV 
FROM DISK = 'C:\Users\Admin\Desktop\kk\Log.bak'

-----------------------------------Cursor-----------------------------


-------------------Thêm một môn học-------------------------
DECLARE CUR_INSERT_MONHOC CURSOR FOR
SELECT MaMH,TenMH,SoTinChi
FROM MONHOC;

OPEN CUR_INSERT_MONHOC;

-- Khai báo biến để lưu trữ dữ liệu từ CURSOR
DECLARE @MaMH nvarchar(10), @TenMH nvarchar(100), @SoTinChi int;

-- Fetch dữ liệu từ CURSOR
FETCH NEXT FROM CUR_INSERT_MONHOC INTO @MaMH, @TenMH, @SoTinChi;

-- Vòng lặp để cập nhật bảng
WHILE @@FETCH_STATUS = 0
BEGIN
    -- Câu lệnh INSERT
	INSERT INTO MONHOC(MaMH, TenMH, SoTinChi) 
	VALUES ('BVBF3438690', N'Hệ quản trị cơ sở dữ liệu', 3)
...
CLOSE CUR_INSERT_MONHOC;
DEALLOCATE CUR_INSERT_MONHOC;
...
-------------------Sửa một môn học-------------------------
DECLARE CUR_UPDATE_MONHOC CURSOR FOR
SELECT
    MaMH,
    TenMH,
    SoTinChi
...
OPEN CUR_UPDATE_MONHOC;

-- Khai báo biến để lưu trữ dữ liệu từ CURSOR
DECLARE @MaMH nvarchar(10), @TenMH nvarchar(100), @SoTinChi int;

-- Fetch dữ liệu từ CURSOR
FETCH NEXT FROM CUR_UPDATE_MONHOC INTO @MaMH, @TenMH, @SoTinChi;
...
BEGIN
    ...
    UPDATE MONHOC
    SET
        TenMH = N'Lập trình hướng đối tượng',
        SoTinChi = 3
    WHERE
        MaMH = 'APCM230307';
...
-- Đóng và giải phóng CURSOR
CLOSE CUR_UPDATE_MONHOC;
DEALLOCATE CUR_UPDATE_MONHOC;
...
-------------------Xóa một môn học-------------------------
DECLARE CUR_DELETE_MONHOC CURSOR FOR
SELECT
    MaMH,
    TenMH,
    SoTinChi
...
OPEN CUR_DELETE_MONHOC;

-- Khai báo biến để lưu trữ dữ liệu từ CURSOR
DECLARE @MaMH nvarchar(10), @TenMH nvarchar(100), @SoTinChi int;

-- Fetch dữ liệu từ CURSOR
FETCH NEXT FROM CUR_DELETE_MONHOC INTO @MaMH, @TenMH, @SoTinChi;
...
-- Đóng và giải phóng CURSOR
CLOSE CUR_DELETE_MONHOC;
DEALLOCATE CUR_DELETE_MONHOC;
...
-----------tạo tài khoản user và login
create login phat2 with password='1234'
create user sinhvien for login phat2

-----------Hủy tài khoản user và login
sp_droplogin 'phat2'
sp_dropuser 'sinhvien'


-----4.1 Quyền thao tác trên đối tượng
----------Cấp quyền
GRANT SELECT,INSERT,UPDATE ON LOPHOC TO sinhvien
---------Thu hồi quyền đã cấp
REVOKE SELECT,INSERT,UPDATE ON LOPHOC TO sinhvien
---------Từ chối quyền
DENY  SELECT,INSERT,UPDATE ON LOPHOC TO sinhvien

-------4.2 Quyền định nghĩa đối tượng
GRANT CREATE TABLE 
TO sinhvien

EXEC sp_helprotect @username = 'sinhvien';
-------4.3 Quản lý quyền thông qua nhóm quyền
-----thêm nhóm quyền sysadmin
sp_addsrvrolemember 'phat2','Sysadmin'
----- hủy nhóm quyền sysadmin
sp_dropsrvrolemember 'phat2','Sysadmin'

-------Tạo nhóm quyền
sp_addrole 'sv'
-------Thêm tài khoản người dùng vào nhóm quyền
sp_addrolemember 'sv','sinhvien'
-------Xóa người dùng khỏi nhóm quyền
sp_droprolemember 'sv','sinhvien'
-------Hủy nhóm quyền
sp_droprole 'sv'
